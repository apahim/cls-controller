...
func (c *Controller) HandleClusterEvent(event *sdk.ClusterEvent) error {
	...
	// If no resources are defined, run as a status-only controller (e.g., version resolution)
	if len(c.controllerConfig.Spec.Resources) == 0 {
		return c.handleStatusOnlyEvent(event, cluster)
	}
...
func (c *Controller) enrichClusterWithControllerStatuses(ctx context.Context, apiClient sdk.APIClient, cluster *sdk.Cluster) {
	...
	// Ensure cluster has a status object
...
// handleStatusOnlyEvent handles cluster events for controllers with no resources.
// Currently used for version resolution: reads release.version from the cluster spec,
// resolves it to a release image via Cincinnati, and reports the result as a status condition.
func (c *Controller) handleStatusOnlyEvent(event *sdk.ClusterEvent, cluster *sdk.Cluster) error {
	log := c.logger.With(
		zap.String("cluster_id", event.ClusterID),
		zap.String("event_type", event.Type),
	)
	...
	if c.cincinnatiClient == nil {
		log.Debug("No Cincinnati client configured, nothing to do for status-only controller")
		return nil
	}
